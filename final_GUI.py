...
from PyQt5.QtWidgets import (QWidget, QLabel,
    QComboBox, QApplication,QPushButton,QGridLayout)
...
class BeamGUI(QWidget):
    ...
    def initUI(self):
        ...
        self.goButtons[0].move(cols[0],rows[1])
        self.goButtons[1].move(cols[4],rows[1])
        ...
        self.cancelButtons[0].move(cols[0],rows[2])
        self.cancelButtons[1].move(cols[4],rows[2])
        ...
        self.statusLbls[0].move(cols[1], rows[2])
        self.statusLbls[1].move(cols[5], rows[2])
    ...
    def runTrials(self, a):
        ...
        for n in a:
            ...
            if self.materialTypes[n] == "- - -" or self.bodyTypes[n] == "- - -" or self.spineTypes[n] == "- - -":
                self.statusLbls[n].setText("Set Body, Spine, and Material")
                self.statusLbls[n].adjustSize()
                run = False
            else:
            ...
            for n in a:
                ...
                #Get the time of the beam break
                    ...
                    #Calculate the speed between the two beams
                        speedArrays[n][state-1] = distArrays[n][state-1]/(timeArrays[n][state]-timeArrays[n][state-1])
                    ...
                    if state > 1 and state % 2 == 1:
                        ...
                        accelArrays[n][int(state/2)-1] = (speedArrays[n][state-1]-speedArrays[n][state-3])/(timeArrays[n][state]-timeArrays[n][state-2])
                        print(accelArrays[n][int(state/2)-1])
